...
export class AuthService {
  ...
  login(email: string, password: string): Observable<boolean> {
    ...
    return this.httpClient.post<LoginResponse>(url, body).pipe(
      ...
      catchError((error) => {
        // Trae el error del backend
        console.log({ error });
        return throwError(() => 'Credenciales invalidas'); // Mandamos un mensaje personalizado en el subscribe(({error}) => {})
      })
    );
  }
  ...
  checkAuthStatus(): Observable<boolean> {
    ...
    return this.httpClient.get<CheckTokenResponse>(url, { headers }).pipe(
      ...
      catchError(() => {
        ...
        return throwError(() => 'Su sesión expiró');
      })
    );
  }
